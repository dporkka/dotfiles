...
-- Host profile tunables (generated by dotfiles/scripts/host-profile.sh), loaded
-- once; absent or unreadable → fall back to the hardcoded default below.
local avante_max_tokens = 16000
do
  local ok, tunables = pcall(dofile, vim.fn.expand("~/.local/state/nvim/tunables.lua"))
  if ok and type(tunables) == "table" and type(tunables.avante_max_tokens) == "number" then
    avante_max_tokens = tunables.avante_max_tokens
  end
end
    ...
    opts = {
      ...
      providers = {
        claude = {
          ...
          extra_request_body = {
            temperature = 0,
            max_tokens = avante_max_tokens, -- was 32000; halves worst-case context memory
          },
        },
